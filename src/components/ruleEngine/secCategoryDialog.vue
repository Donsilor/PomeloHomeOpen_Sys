...
import { addSecCategory, editSecCategory, secCategoryDetail } from '@/api/categoryManager'
export default {
  props: {
    addDialogVisible: {
      type: Boolean,
  ...
  created() {
    ...
    if (this.propData.status !== 0) {
      ...
      secCategoryDetail({ params }).then((res) => {
      ...
      this.$refs.ruleForm.validate((valid) => {
        if (valid) {
          ...
          if (this.propData.status) {
            ...
            editSecCategory({ params }).then((res) => {
            ...
            const params = Object.assign({}, this.form)
            addSecCategory({ params }).then((res) => {
              if (res.data.code === 200 || res.data.code === 0) {
                this.$message({
                  type: 'success',
                  message: '新增成功'
